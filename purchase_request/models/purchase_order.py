...
class AccountMove(models.Model):
    ...
    no_faktur = fields.Many2one('purchase.order', 'no PO')
    no_faktur_related = fields.Char("no faktur" ,compute="_getNoFaktur",store=True, readonly=False)
    request_note = fields.Text("Request Note")
    back_date = fields.Date("Tanggal Kembali", compute="_getBackDate", store=True, readonly=False)
    ...
    def _getNoFaktur(self):
        for rec in self:
            nofaktur = rec.invoice_origin
            order = self.env["purchase.order"].search([("name", "=", nofaktur)])
            rec.no_faktur_related = order.nomor_faktur

    @api.depends('date', 'invoice_payment_term_id')
    def _getBackDate(self):
        for rec in self:
            getDueDate = rec.invoice_payment_term_id.name


            if int(rec.invoice_payment_term_id) > 1 and int(rec.invoice_payment_term_id) < 6:
                trimString = getDueDate[0:2]
                toInt = int(trimString)
                getDate = rec.date + timedelta(days=toInt)
                rec.back_date = getDate
            elif int(rec.invoice_payment_term_id) == 1:
                rec.back_date = rec.date
            elif int(rec.invoice_payment_term_id) == 6:
                getDate = rec.date + timedelta(days=60)
                rec.back_date = getDate
            elif int(rec.invoice_payment_term_id) == 7:
                getDate = rec.date + timedelta(days=31)
                rec.back_date = getDate
    ...
    def print_faktur(self):
        if self.id:
            return self.env.ref('purchase_request.print_faktur').report_action(self)
